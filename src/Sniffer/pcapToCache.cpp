...
insert_tcp_hex_mysql(int type, string src_ip, string dst_ip, int src_port, int dst_port,   unsigned char *payload, unsigned long seq, int len,int fin)
{
    ...
    insert_sql ="INSERT INTO testPcapHex VALUES( NULL, '"+ stype +"', '"+  src_ip +"', '"+  dst_ip +"', '"+  ssrc_port +"', '"+ sdst_port +"', '"+  strHexContent +"', '"+ sseq +"','"+ slen +"','"+ sfin +"');";

    int res =mysql_real_query( pcap_conn , insert_sql.c_str() , insert_sql.length() );

    if (!res)
    {
        tcp_num_count++;
        printf("Inserted %lu rows\n", (unsigned long)mysql_affected_rows(pcap_conn));
    }
    else
    {
        fprintf(stderr, "Insert error %d: %s\n", mysql_errno(pcap_conn),
                mysql_error(pcap_conn));
    }
   ...
   */
    if (proto_flag == 0)
    {
        ...
        if (size_payload > 0)
        {
           ...
           // unsigned char* content=(unsigned char*)malloc(size_payload*sizeof(unsigned char));
           // memcpy(content, payload, size_payload);

            pthread_mutex_lock(&tcpMut);

            sql_put_tcp_cache(inet_addr(inet_ntoa(ip->ip_src)),inet_addr(myip),ntohs (tcp->th_sport),
                              ntohs (tcp->th_dport),payload,ntohl (tcp->th_seq),size_payload,fin);
    ...
    else if (proto_flag == 1)
    {
       ...
       */
        if (size_payload > 0)
        {
           ...
           // char* content=(char*)malloc(size_payload*sizeof(char));
           // memcpy(content, payload, size_payload);

            pthread_mutex_lock(&udpMut);

       sql_put_udp_cache(inet_addr(inet_ntoa(ip->ip_src)),inet_addr(myip),ntohs (udp->sport),
                             ntohs (udp->dport),payload,size_payload);
...
void connectMysql()
{
    ...
    pcap_conn=mysql_init(NULL);

    if(!mysql_real_connect(pcap_conn,"localhost","root","",dataBaseName,0,NULL,0)) //连接test数据库
    {
...
void* get_data_from_pcap(void *arg)
{
    ...
    char *dev = "eth0";                                        /* 捕获设备的名称 | capture device name */
