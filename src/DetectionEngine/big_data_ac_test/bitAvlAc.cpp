/***
bitAcvlBool
g++ -o bitAcvlBool.o bitAcvlBool.cpp
***/
#include    <stdio.h>
#include    <malloc.h>
#include    <memory.h>
...
struct ACVL
{
    ...
    struct ACVL *lchild;
    struct ACVL *rchild;
   //  bitset<bitwidth> bitque;
   bool bitque[bitwidth];
};
...
void insertACVL(int * str,ACNODE *root,int num)
{
    ...
    {
        index = str[i];
       if(p->bitque[bitwidthlow-index]==0)
            // if(queryAvlExist(p->avlRoot,index)==0)
        {
            ...
            newnode->data=index;
            //newnode->bitque=0;
            memset(&newnode->bitque[0],0,sizeof(newnode->bitque[0])*bitwidth);
            newnode->count = 0;
            newnode->avlRoot=NULL;
            p->avlRoot= buildAvl(index,p->avlRoot,newnode);
            p->bitque[bitwidthlow-index]=1;
        }
                ...
                if(queryAvlExist(tt->avlRoot,t)==1)
                {
                    ...
                    else
                    {
                        ...
                        while (p != NULL)
                        {
                            tt=temp;
                            pNext=p;
                          //  if(pNext->bitque[map[1][t]]==1)
                            if(pNext->bitque[bitwidthlow-t]==1)
...
int queryStr(ACNODE *root)
{
    ...
    while(!feof(fp))
    {
       ...
       // while ((tt->bitque[map[1][index]]==0)&&tt!= root)
        while ((tt->bitque[bitwidthlow-index]==0)&&tt!= root)
        {
            p = p->fail;
            tt=p;
        }
...
int main()
{
    ...
    root->avlRoot=NULL;
    root->data=-2;
    memset(&root->bitque[0],0,sizeof(root->bitque[0])*bitwidth);
    root->count = 0;
    start = clock();
